<?php
    ...
    $numPowerups = mysqli_query($connection, "SELECT foodPowerup FROM USER WHERE user_id = '$userID'");
    if (!$numPowerups) {
        printf("Error: %s\n", mysqli_error($connection));
        exit();
    }
    $line = mysqli_fetch_array($numPowerups, MYSQLI_ASSOC);
    $numPowerups = $line['foodPowerup'];
    ...
    if ($numPowerups == 0) {
        ...
        $deletionQuery = mysqli_query($connection, "UPDATE USER SET foodPowerup = $numPowerups - 1 WHERE user_id='$userID'");
